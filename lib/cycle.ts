import { fetchGoldPrice, fetchGoldHistory, fetchGoldNews, computeTechnicalStats } from "./marketData";
...
import { loadPortfolio, savePortfolio, saveLatestState, pushJournalEntry, pushLogs, loadJournal, loadLogs, getCachedOrFetch } from "./store";
...
export async function runCycle(): Promise<CycleState> {
  ...
  // 1b. Historical prices + news — both optional and independently cached
  // (see getCachedOrFetch in store.ts) so they stay well within their free
  // API tiers' limits. Each degrades gracefully — same "admit the gap rather
  // than invent data" pattern as everywhere else in this pipeline — if its
  // key isn't configured or the call fails.
  let technicalStats: TechnicalStats | null = null;
  let historyNote = "no METALPRICE_API_KEY set";
  try {
    const history = await getCachedOrFetch("history", () => fetchGoldHistory(30));
    technicalStats = computeTechnicalStats(history);
    historyNote = `${history.length}-day history loaded (metalpriceapi.com)`;
    logs.push(log("Market Data", "INFO", `Loaded ${history.length}-day gold price history from metalpriceapi.com.`));
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    historyNote = `history unavailable: ${message}`;
    logs.push(log("Market Data", "WARN", `No historical price data this cycle: ${message}`));
  }

  let newsHeadlines: NewsHeadline[] = [];
  let newsNote = "no FINNHUB_API_KEY set";
  try {
    newsHeadlines = await getCachedOrFetch("news", () => fetchGoldNews(8));
    newsNote = `${newsHeadlines.length} headlines loaded (finnhub.io)`;
    logs.push(log("Market Data", "INFO", `Loaded ${newsHeadlines.length} recent headlines from Finnhub.`));
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    newsNote = `news unavailable: ${message}`;
    logs.push(log("Market Data", "WARN", `No live news feed this cycle: ${message}`));
  }
  ...
  agents.push({
    ...
    status: "RUNNING",
    confidence: null,
    task: "Fetch current XAU/USD spot price, recent history, and relevant headlines.",
    output: `${gold.price} ${gold.currency} as of ${gold.updatedAt} (${gold.source}). ${historyNote}. ${newsNote}.`,
    tookMs: 0,
  });
  ...
  const [macro, technical, news] = await Promise.all([
    runMacroAgent(gold),
    runTechnicalAgent(gold, technicalStats),
    runNewsAgent(gold, newsHeadlines),
  ]);
